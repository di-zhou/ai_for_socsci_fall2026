...
::: {style="font-size: 0.8em;"}
::: {.callout-note}
# **Overfitting**: When a model fits the training data "too well," learning even the random noise in it, resulting in poor performance on unseen test data.
:::
:::


## Viewing Statistical Concepts through the ML Lens

| Statistical Inference | Machine Learning |
|---|---|
| Regression model | **Model** |
| Independent variables / predictors | **Features** |
| Dependent variable / outcome | **Label** (or target) |
| Intercept / constant | **Bias** |
| Coefficients | **Weights** |
| Squared error (SSE, MSE) | **Loss** |
| Fitting a model | **Training** a model |
| Model fit (R², AIC) | Model performance (accuracy, F1, etc.) |
...
|--|---|---|
| **Ground truth** | **Known** — we have labeled data | **Unknown** — no labels |
| **Goal** | **Predict** the label correctly | **Discover** patterns or structure |
| **Data** | Labeled training and test sets | Features only, no labels |
| **Examples** | Linear regression, logistic regression, tree-based models | Principle Component Analysis (PCA), k-means clustering |
:::
...
![](images/sml_vs_usml.png){width="100%"}
